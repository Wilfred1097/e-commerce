...
<main class="main">
          ...
          <div class="card checkout-card">
            <div class="card-body">
              ...
              <div class="payment-options mb-4">
                ...
                <div class="form-check mb-3">
                  <input class="form-check-input" type="radio" name="paymentMethod" id="paynowOption" value="Gcash">
                  <label class="form-check-label" for="paynowOption">
                    <strong>Pay Now (Gcash)</strong>
                    <div class="text-muted small">Pay your order now</div>
                  </label>
                </div>
              ...
              <div class="mb-4 justify-content-center" id="paynowContainer" style="display: none;">
                <label for="paynow" class="form-label d-block mb-2">Please Pay your bill here and enter Ref No. below:</label>
                <img id="paynow" src="assets/img/gcash.jpg" alt="Payment Instructions" class="img-fluid" width="50%">
                <input type="text" class="form-control mt-2" placeholder="Enter reference number here" id="refNo" name="refNo" required>
              </div>
  ...
  <script>
      ...
      radio.addEventListener('change', function () {
        ...
        document.getElementById('paynowContainer').style.display = (method === 'COD' && this.id === 'paynowOption') ? 'block' : 'none';
        document.getElementById('addressContainer').style.display = (method === 'COD' && this.id === 'codOption') ? 'block' : 'none';
      });
    });
    ...
    const paynowRadio = document.getElementById('paynowOption');
    ...
    function updateSummaryWithShipping(subtotal, shippingFee = 0) {
    ...
    paymentRadios.forEach(radio => {
      radio.addEventListener('change', function () {
        const method = this.value;
        if (method === 'Pickup') {
          document.getElementById('paynowContainer').style.display = 'none';
          document.getElementById('addressContainer').style.display = 'none';
          updateSummaryWithShipping(subtotal, 0);
        } else if (method === 'COD') {
          document.getElementById('paynowContainer').style.display = 'none';
          document.getElementById('addressContainer').style.display = 'block';
          updateSummaryWithShipping(subtotal, 100); // example fee
        } else if (method === 'Gcash') {
          document.getElementById('paynowContainer').style.display = 'block';
          document.getElementById('addressContainer').style.display = 'block';
          updateSummaryWithShipping(subtotal, 0); // example fee
        }
      });
    });
    ...
    pickupRadio.addEventListener('change', toggleOptions);
    codRadio.addEventListener('change', toggleOptions);
    paynowRadio.addEventListener('change', toggleOptions);
    ...
    function toggleOptions() {
      ...
      if (document.getElementById('paynowOption').checked) {
    ...
    document.getElementById('placeOrderBtn').addEventListener('click', () => {
      ...
      if (paymentMethod === 'COD' && addressField.value.trim() === '') {
        Swal.fire({
          icon: 'warning',
